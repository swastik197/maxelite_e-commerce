...
const AuthModal = ({ isOpen, onClose }) => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setTimeout(() => {
          onClose();
          window.location.reload(); // Quick way to reload state, or we could just rely on context checking
        }, 1000);
      } else {
  ...
  return (
    <div className='fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 sm:p-6'>
      <div className='relative w-full max-w-4xl max-h-[calc(100vh-2rem)] sm:max-h-[90vh] flex overflow-hidden shadow-2xl rounded-2xl bg-white'>
        {/* Close Button */}
        <button 
          onClick={onClose}
          className='absolute top-3 right-3 sm:top-4 sm:right-4 z-20 text-gray-500 hover:text-gray-800 bg-white/50 backdrop-blur rounded-full p-1.5 sm:p-1 transition'
        >
          <CloseIcon />
        </button>
        ...
        {/* Right Side - Form */}
        <div className='w-full lg:w-1/2 flex items-center justify-center p-6 sm:p-8 lg:p-12 overflow-y-auto w-full h-full bg-gray-50'>
